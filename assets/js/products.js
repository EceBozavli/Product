...
const productList = document.querySelector('.productList');
    ...
    filteredProducts.forEach(product => {
      ...
      `
        <div class="cards">
          ...
          <h3 class="prTitle">${product.title}</h3>
          <p class="prCategory"><strong>Brand:</strong> ${product.category}</p>
          <p>${product.description}</p>
          <p class="prPrice">$ ${product.price}</p>
          <p class="prStock"><strong>Stock:</strong> ${product.stock} piece</p>
          <p class="prEtiket"> ${product.tags.join(', ')}</p> 
        </div>
      `;
  ...
  function searchProducts() {
    ...
    const filteredProducts = products.filter(product => {
      return (
        ...
        product.tags.some(tag => tag.toLowerCase().includes(searchTerm)) // Etiketlerde arama
        //.some(): JavaScript dizileri üzerinde kullanılan some metodu, dizinin en az bir elemanının belirtilen şartı sağlayıp sağlamadığını kontrol eder. Eğer en az bir eleman şartı sağlıyorsa, true döner, yoksa false.
      );
    });
  ...
  // Arama kutusunda her tuşa basıldığında searchProducts fonksiyonunu çağır
  searchInput.addEventListener('input', searchProducts);

  // Sayfa ilk yüklendiğinde tüm ürünleri listele
